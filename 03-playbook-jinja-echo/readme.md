# Ansible: Run Playbook with Jinja2 Template on Worker Nodes

Running an Ansible Playbook using a Jinja2 template variable to execute an echo command and print the output on managed nodes using Ansible from a master EC2 instance.
...
Create a new playbook file named `greetings.yml` using a text editor such as Vim:
...
- name: Greetings From Nur
  ...
  tasks:
    - name: Run echo command
      command: echo "Hello From {{ student_name }}"
...
Execute the playbook to run the task on the managed nodes and display the registered echo output:
